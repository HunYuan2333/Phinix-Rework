...
- 聊天消息列表已具备动态行高、布局缓存和可见区域绘制，可作为大型列表参考实现。
- 红包列表已开始使用可见行裁剪。
...
1. 主聊天输入区在按钮和输入框无法并排时切为两行。
2. 回复条根据文本动态测高并缓存，关闭按钮始终可达。
3. 消息列表保留现有动态高度/虚拟化，迁移到共享 `VirtualListLayout` 时先做等价测试。
4. 在线用户侧栏声明最小/首选宽度；窄屏允许 Host 折叠为侧栏抽屉或单独侧栏页。
5. 通知行由固定 46px 改为缓存动态行高，并虚拟化最多 100 条通知。
6. “全部已读”等操作在窄侧栏进入溢出菜单或换行。
7. 公告高度按当前宽度测量并缓存，不允许覆盖主 Tab。
8. 设置面板验证 Listing 在窄宽度和长文本下可用。
